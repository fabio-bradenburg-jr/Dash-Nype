...
export async function PATCH(request) {
  try {
    ...
    const connection = await updateWorkspaceGoogleCalendarSelection(supabase, accessContext.workspaceId, {
...
export async function DELETE() {
  try {
    const { supabase, accessContext } = await getAuthorizedGoogleCalendarContext({ requireEdit: true })
    await deleteWorkspaceGoogleCalendarConnection(supabase, accessContext.workspaceId)
